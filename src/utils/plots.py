...
def plot_node_forecasting(pred, true, idx_range=(0, 500), labels=None):
    ...
    for idx in range(n_nodes):
        axes[idx//n_cols, idx%n_cols].plot(true[n_nodes*idx_range[0]+idx:n_nodes*idx_range[1]:n_nodes], label='true')
        axes[idx//n_cols, idx%n_cols].plot(pred[n_nodes*idx_range[0]+idx:n_nodes*idx_range[1]:n_nodes], label='pred')
        axes[idx//n_cols, idx%n_cols].legend()
        if labels is not None:
            axes[idx//n_cols, idx%n_cols].set_title(labels[idx])
    return f
