...
def get_detection_metrics(true_labels, pred_labels, score, fault_labels, threshold, evaluate_pak=False):
    try:
...
def get_prediction_scores(
    ...
    if anomaly_score_func in ad_arg_mapping:
        ...
        if anomaly_score_func == 'grelen_ad':
        ...
        elif anomaly_score_func == 'usad_ad':
            ...
            val_score, test_score = get_gdn_ad_score(val_trues=val_trues, val_preds=val_preds, 
        ...
        raise ValueError(f'Unknown anomaly score function: {anomaly_score_func}. '+
                         'Should be one of: base, node_scaled, node_scaled_combined, node_scaled_topk, att_scaled, gdn_ad, grelen_ad, usad_ad.')
    ...
    out.update({
        ...
        'threshold_best': round(threshold_best_pr, cfg.round),
